...
const App = () => {
    // Fetching methods and state from Project Context
    const {
        projects,
        expenses,
        users,
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                // Fetch projects, expenses, and users when the app loads
                await Promise.all([
                    projects,
                    expenses,
                    users
    ...
    // Update: Ensure personalized greeting and token persistence for user sessions
    const handleSignUpSuccess = (user) => {
        handleLoginSuccess({ data: { token: localStorage.getItem('authToken'), user } });
        showToast(`Welcome, ${user.name || 'User'}!`);
        navigate('/projects');
    };
